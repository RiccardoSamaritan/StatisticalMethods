...
gam_1 <- gam(cnt ~ hr + temp + workday_f + weather_f + yr_f, family = poisson, data = train_data)
summary(gam_1)

# GAM with spline smooth for hr and temp (Poisson) - captures non-linearity, cyclic cubic spline
gam_2 <- gam(cnt ~ s(hr, bs = "cc", k = 24) + s(temp) + workday_f + weather_f+ yr_f, family = poisson, data = train_data)
summary(gam_2)

# GAM with hr-workingday interaction (Poisson) - hour effect depends on day type
gam_3 <- gam(cnt ~ s(hr, bs = 'cc', by = workingday) + s(temp) + weather_f + yr_f, family = poisson, data = train_data)
summary(gam_3)

# GAM Negative Binomial with interaction - handles overdispersion in count data
gam_nb <- gam(cnt ~ s(hr, bs = 'cc', by = workingday) + s(temp) + weather_f + yr_f, family = nb(), data = train_data)
summary(gam_nb)
...
Per confermare che il modello più complesso (M3) sia effettivamente necessario, eseguiamo un test ANOVA. (n.d.r. si potrebbe fare anche con gli altri ma sarebbe ridondante)
```{r anova-test}
...
gam_final_ciclica <- gam(cnt ~ s(hr, bs = 'cc', k = 24) + weather_f + yr_f, family = nb(), data = train_data)
...
```{r 3d-interaction}
# GAM NB con interazione hr-workday per superficie 3D - visualizza pattern differenziati questo xe best
# senza anno
...
get_metrics <- function(model, data, target_col) {
  ...
  # Usiamo type = "response" per ottenere i conteggi previsti (non il log)
...
performance_comparison <- data.frame(
  Modello = c("Senza Anno (Best)", "Con Anno (Year)"),
...
```
```{r}
# generiamo altri modelli mars a partire da quello di prima e confrontiamoli

# mars model senza interazioni (degree = 1)
mars_model_no_int <- earth(cnt ~ hr + temp + workday_f + weather_f + season_f + yr_f, data = train_data, degree = 1)

# mars model senza l'uso delle ore
...
mars_comparison <- data.frame(
  Modello = c("MARS senza Interazioni", "MARS senza Ore", "MARS con Interazioni"),
  GCV = c(gcv2, gcv3, gcv1)  # Ordine: no_int, no_hr, con_int
)
kable(mars_comparison, caption = "Confronto GCV per Modelli MARS")
```

```{r CV}
# Confrontiamo i modelli rispetto alla capacità predittiva puntuale, misurata via RMSE
library(dplyr)

# Creiamo i fold (es. 5 fold)
set.seed(6767)
folds <- sample(rep(1:5, length.out = nrow(train_data)))

# Inizializziamo i vettori per i risultati
rmse_mars <- numeric(5)
rmse_gam  <- numeric(5)
...
for(i in 1:5) {
  # Divisione Train/Validation interna
  ...
  # Fit GAM (usiamo REML che è più robusto per Negative Binomial)
...
# Risultato finale: Media degli RMSE e media dei MAE
cat('mars',mean(rmse_mars), '\n')
cat('gam', mean(rmse_gam), '\n')

cat('mars', mean(mae_mars), '\n')
cat('gam', mean(mae_gam), '\n')

# Verifica quale modello viola meno le proprie assunzioni
par(mfrow = c(1, 2))
plot(pred_mars, cv_val$cnt - pred_mars)  # Omoschedasticità?
plot(pred_gam, cv_val$cnt - pred_gam)    # Eteroschedasticità quadratica?
```
```{r visualizzazione gam model spline singole}
# Visualizzazione degli effetti spline per il GAM gam_interaction_year
plot(gam_interaction_year, select = 1, main = "Effetto Spline (hr) con workday0")
plot(gam_interaction_year, select = 2, main = "Effetto Spline (hr) con workday1")
plot(gam_interaction_year, select = 3, main = "Effetto Spline (temp)")

# select = 1: Spline di hr per il primo livello di workday_f.

# select = 2: Spline di hr per il secondo livello di workday_f.

# select = 3: Spline della temperatura s(temp).
```


# 4. Valutazione Finale su Test Set (Metrics)

Confrontiamo tutti i modelli su dati mai visti (Test Set) usando MSE, RMSE e R². questo non va ancora bene rick
```{r final-evaluation}
...
new_data <- data.frame(
  hr = 8,
  temp = 0.5,
  weather_f = factor("Clear", levels = levels(df$weather_f)),
  workday_f = factor("Working day", levels = levels(df$workday_f)), 
  yr_f = factor("2012", levels = levels(df$yr_f))
)

predizione_test <- predict(gam_interaction_year, new_data, type = "response")
print(predizione_test)
```
