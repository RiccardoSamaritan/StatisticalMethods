...
metrics_best <- get_metrics(gam_interaction_best, test_data, "cnt")
...
# 4. Valutazione Finale su Test Set (Metrics) questa parte è in più era solo una prova per le metriche post train test validation
...
ggplot(plot_long, aes(x = Time, y = Valore, color = Modello, linetype = Modello)) +
  ...
  theme_minimal()
```
prova di una predizione così a caso
```{r prova predizione}
new_data <- data.frame(
  hr = 8,
  temp = 0.5,
