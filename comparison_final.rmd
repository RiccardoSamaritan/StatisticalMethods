...
results <- list(
  ...
  Tree_no_pruned = evaluate_model(tree_model, test_data, "tree"),
...
```
```{r}
# scriviamo la lista dei vari aic dei modelli ordinata
aic_values <- c(
  ...
  MARS = NA, # MARS non ha AIC
  Tree = NA, # Tree non ha AIC
  Random_Forest = NA # Random Forest non ha AIC
)
aic_df <- data.frame(Model = names(aic_values), AIC = aic_values)
aic_df <- aic_df[order(aic_df$AIC), ]
kable(aic_df, caption = "AIC dei modelli (ordinato)")
```
